...
async fn main() -> Result<()> {
    process(
        ...
        async |transactions, conn_pool| {
            ...
            let processed_data = transactions
                ...
                .map(|txn| {
                    ...
                    for event in raw_events {
                        ...
                        if event_type.contains("0x66c4ec614f237de2470e107a17329e17d2e9d04bd6f609bdb7f7b52ae24c957c") {
                            info!("Found Kizo event at version {}: type={}, data={}", txn_version, event_type, event.data);
                        }
                        ...
                        match event_type {
                            MARKET_CREATED_EVENT => {
                                match parse_event_data::<MarketCreatedEvent>(event) {
                                    Some(market_event) => {
                                        markets.push(Market::from_event(&market_event, txn_version, block_height));
                                        info!("Successfully parsed market at version {}", txn_version);
                                    }
                                    None => {
                                        error!("Failed to parse MarketCreatedEvent at version {}: {}", txn_version, event.data);
                                    }
                                }
                            }
                            BET_PLACED_EVENT => {
                                if let Some(bet_event) = parse_event_data::<BetPlacedEvent>(event) {
                                    bets.push(Bet::from_event(&bet_event, txn_version, block_height));
                                }
                            }
                            MARKET_RESOLVED_EVENT => {
                                if let Some(resolution_event) = parse_event_data::<MarketResolvedEvent>(event) {
                                    market_resolutions.push(MarketResolution::from_event(&resolution_event, txn_version, block_height));
                                }
                            }
                            WINNINGS_CLAIMED_EVENT => {
                                if let Some(claim_event) = parse_event_data::<WinningsClaimedEvent>(event) {
                                    winnings_claims.push(NewWinningsClaim::from_event(&claim_event, txn_version, block_height));
                                }
                            }
                            YIELD_DEPOSITED_EVENT => {
                                if let Some(deposit_event) = parse_event_data::<YieldDepositedEvent>(event) {
                                    yield_deposits.push(NewYieldDeposit::from_event(&deposit_event, txn_version, block_height));
                                }
                            }
                            PROTOCOL_FEE_COLLECTED_EVENT => {
                                if let Some(fee_event) = parse_event_data::<ProtocolFeeCollectedEvent>(event) {
                                    protocol_fees.push(NewProtocolFee::from_event(&fee_event, txn_version, block_height));
                                }
                            }
                            _ => {
                    ...
                    (markets, bets, market_resolutions, winnings_claims, yield_deposits, protocol_fees)
                })
                .collect::<Vec<_>>();
            ...
            let total_new_items = markets.len() + bets.len() + market_resolutions.len() + 
                                 winnings_claims.len() + yield_deposits.len() + protocol_fees.len();
...
async fn trigger_backend_sync(total_items: usize) {
    ...
    info!("🔔 Triggering backend sync for {} new items: {}", total_items, backend_url);
    ...
    tokio::spawn(async move {
        match client.post(&backend_url_clone)
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    warn!("⚠️  Backend sync endpoint returned status: {}", response.status());
                }
            }
            Err(e) => {
